    ...
    if(data.esFeriado != null){            
      ...
      for (i = 0; i < count; i++) {    
         ...
         if(data.turnosPaciente[i].dni == 99999) {
            var paciente = '<tr><th scope="row">'+contador+'</th><td>'+tipoTurnoText+'</td><td>'+data.turnosPaciente[i].horario+'</td><td></td><td>Cancelar</td><td></td><td></td></tr>';
         } else {
           if(data.primerControl == 1){                       
            //alert(data.turnosPaciente[i].libre+" - "+data.turnosPaciente[i].horario);
            if(data.turnosPaciente[i].libre == 0){  
                var paciente = '<tr><th scope="row">'+contador+'</th><td>'+tipoTurnoText+'</td><td>'+data.turnosPaciente[i].horario+'</td><td>'+data.turnosPaciente[i].dni+'</td><td><button onClick=reservarTurnoPrimerControl("'+data.turnosPaciente[i].horario+'","'+data.turnosPaciente[i].horario2+'") class="rodri_button_aceptar">REGISTRAR</button></td><td></td><td></td></tr>';                                                                  
              } else {
                var paciente = '<tr><td>'+contador+'</td><td>'+tipoTurnoText+'</td><td>'+data.turnosPaciente[i].horario+'</td><td>'+data.turnosPaciente[i].dni+'</td><td>'+data.turnosPaciente[i].nombre+'</td><td><button onClick=modalCancelarTurno("'+data.turnosPaciente[i].horario+'") class="rodri_button_cancelar_no">X</button></td><td><button onClick=modalModificarTurno("'+data.turnosPaciente[i].horario+'","'+data.turnosPaciente[i].trid+'","'+data.turnosPaciente[i].dni+'") class="rodri_button_aceptar_si">M</button></td></tr>';                                                      
              }
          }
          else{                      
            if(data.turnosPaciente[i].libre == 0){                      
             var paciente = '<tr><th scope="row">' + contador + '</th><td>'+tipoTurnoText+'</td><td>' + data.turnosPaciente[i].horario + '</td><td>' + data.turnosPaciente[i].dni + '</td><td><button onClick="reservarTurno(\'' + data.turnosPaciente[i].horario + '\', \'' + tipoTurno + '\')" class="rodri_button_aceptar">REGISTRAR</button></td><td></td><td></td></tr>';
            } else {
              var paciente = '<tr><td>'+contador+'</td><td>'+tipoTurnoText+'</td><td>'+data.turnosPaciente[i].horario+'</td><td>'+data.turnosPaciente[i].dni+'</td><td>'+data.turnosPaciente[i].nombre+'</td><td><button onClick=modalCancelarTurno("'+data.turnosPaciente[i].horario+'") class="rodri_button_cancelar_no">X</button></td><td><button onClick=modalModificarTurno("'+data.turnosPaciente[i].horario+'","'+data.turnosPaciente[i].trid+'","'+data.turnosPaciente[i].dni+'") class="rodri_button_aceptar_si">M</button></td></tr>';                                                          
            }
          }
