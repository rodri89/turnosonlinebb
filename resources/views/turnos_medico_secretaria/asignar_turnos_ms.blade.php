     ...
     $.ajax({
           ...
           success:function(data){          
              var contador = 0;
                ...
                generarTabla(data);                
                document.getElementById("fechaLibreDisponible").value = data.fechaLibreMasCercana;
           }
        });
  }
  ...
  function registrarPaciente(){
      ...
      if(isNaN(dni)||(isNaN(telefono))){
        alert("El DNI y el telefono deben estar compuestos solo por numeros.");
           ...
           data:{dni :dni,nombre:nombre,apellido:apellido,fecha_nacimiento:fecha_nacimiento,telefono:telefono,mail:mail,obra_social:obrasocial,numero_afiliado:numero_afiliado,plan:plan, consultorio:consultorio, domicilio:domicilio, localidad:localidad, _token: '{{csrf_token()}}'},
		       success:function(data){      
  ...
  function registrarTurno(){  			
    ...
    if((primerControl) && (moduloPrimerControlDoble == 1)) {
      $.ajax({
         ...
         data:{paciente_id :paciente_id, medico_id:medico_id, consultorio:consultorio, fechaTurno:fechaTurno, horario:horario, horario2:horario2, primerControl:primerControlNum, _token: '{{csrf_token()}}'},
         success:function(data){
            //alert(data.dat_pac);
	   ...
	   $.ajax({
	       ...
	       data:{paciente_id :paciente_id, medico_id:medico_id, consultorio:consultorio, fechaTurno:fechaTurno, horario:horario, primerControl:primerControlNum, _token: '{{csrf_token()}}'},
	       success:function(data){
            //alert(data.dat_pac);
		       if(data.turnoRegistrado == 1) {        
		       		$("#modalTurnoOk").modal();		       			       			       				     	       		 
