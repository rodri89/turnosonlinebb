...
@section('contenedor')
<?php $lucas_sosa_id = 13; // 13 es en produccion y 12 en local?>
...
<div class="row">
  ...
  <h2 class="medicoTituloTextoCel">Listado Pacientes del dia</h2><br>      
  <div class="col-md-2">
    <form> 
      @csrf                         
        @if($medico->id == $lucas_sosa_id || $medico->id == $marcelo_cardiologo_id || $medico->id == 11)
      ...
      <table class="table table-condensed" id="tabla_pacientes" name="tabla_pacientes">
       <thead>
          <tr>
            ...
            <th class="editText" scope="col">Telefono</th>
            <th class="editText" scope="col">Primer Control</th>            
            @if($moduloCajaComentario == 1)
              <th class="editText" scope="col">Caja</th>
              <th class="editText" scope="col">Comentario</th>
            @endif
        ...
        <tbody id="pacientes-list" name="pacientes-list">
          ...
          <tr>
            <th scope="row" class="editText">{{$cont++}}</th>
            <td>{{$tr->horario}}</td>
            ...
            @else
              ...
              <td class="editText"><a type="button" onclick="verDatosPaciente('{{$tr->dni}}')"><u>{{$tr->dni}}</u></a></td>            
              <td class="editText">{{$tr->telefono}}</td>                     
              <td class="editText">{{$tr->primerControl}}</td>                                                                      
              @if($moduloCajaComentario == 1)
                <td class="editText"><input type="text" onchange="guardarCaja('{{$tr->trid}}')" size="8" id="caja{{$tr->trid}}" name="caja" value="{{$tr->caja}}"/></td>                                                                      
              ...
              @if($medico->especialidad == 2)
                @if($tr->tipo_turno == 1)
                  <td>Consulta y ECG</td>
                @endif
                @if($tr->tipo_turno == 2)
                  <td>Ecocardiograma Doppler Color</td>
                @endif
                @if($tr->tipo_turno == 3)
                  <td>Ecodoppler de vasos de cuello</td>
                @endif                
                @if($tr->tipo_turno == $medico->id)
                  <td>PEG</td>
                @endif
              @endif 
              ...
              @if($tr->asistio == 0)                                             
                @if($tr->tipo_turno == $medico->id)                                             
                  <td><button class="rodri_button_aceptar_si" onclick="registrarAsistenciaPeg('{{$tr->trid}}',1)">SI</button>
                  <button class="rodri_button_cancelar_no" onclick="registrarAsistenciaPeg('{{$tr->trid}}',2)">NO</button></td>
                @else
                  <td><button class="rodri_button_aceptar_si" onclick="registrarAsistencia('{{$tr->trid}}',1)">SI</button>
                  <button class="rodri_button_cancelar_no" onclick="registrarAsistencia('{{$tr->trid}}',2)">NO</button></td>
                @endif                        
              @else
                @if($tr->asistio == 1)                                             
                  <td><button class="rodri_button_aceptar_si" disabled>SI</button></td>                      
                @else
                  <td><button class="rodri_button_cancelar_no" disabled>NO</button></td>
                @endif   
              @endif                             
            @endif
          </tr>
...
<script type="text/javascript">  
  ...
  function guardarCaja(caja_id){
    ...
    var dia = document.getElementById("dia").value;    
     $.ajax({
           type:'POST',
           dataType:'JSON',
           url:'/update_caja_listado_pacientes',
  ...
  function registrarAsistencia(turnoRegistradoId, asistio){
    ...
    var consul = document.getElementById("consultorio").value;    
     $.ajax({
           type:'POST',
           dataType:'JSON',
           url:'/medico_registrar_asistencia',
  ...
  function registrarAsistenciaPeg(turnoRegistradoId, asistio){
    //alert("holas "+turnoRegistradoId);    
    var dia = document.getElementById("dia").value;    
    var medico = document.getElementById("medico_id").value;    
    var consul = document.getElementById("consultorio").value;    
  ...
  function cargarTablaCardiologo(data){
    var contador = 0;
    ...
    for (i = 0; i < data.turnosPaciente.length; i++){
        ...
        var tt = data.turnosPaciente[i].tipo_turno;
        var tipo_turno = "Consulta y ECG";
        var asistio = "<td><button class='rodri_button_aceptar_si' onclick='registrarAsistencia("+data.turnosPaciente[i].trid+",1)'>SI</button><button class='rodri_button_cancelar_no' onclick='registrarAsistencia("+data.turnosPaciente[i].trid+",2)'>NO</button></td>";
        if(tt == 2)
          tipo_turno = "Ecocardiograma Doppler Color";
        if(tt == 3)
          tipo_turno = "Ecodoppler de vasos de cuello";
        if(tt == 9){
          tipo_turno = "PEG";
          asistio = "<td><button class='rodri_button_aceptar_si' onclick='registrarAsistenciaPeg("+data.turnosPaciente[i].trid+",1)'>SI</button><button class='rodri_button_cancelar_no' onclick='registrarAsistenciaPeg("+data.turnosPaciente[i].trid+",2)'>NO</button></td>";          
        }

        if(data.moduloCajaComentario == 0){
          if(data.turnosPaciente[i].dni == 99999){
          var paciente = "<tr><td>"+contador+"</td><td>"+data.turnosPaciente[i].horario+"</td><td>Cancelado</td><td></td><td></td><td></td><td></td><td></td></tr>";  
          } else {            
            if(data.turnosPaciente[i].asistio == 0){ // todavia no se realizo accion
            var paciente = "<tr><td>"+contador+"</td><td>"+data.turnosPaciente[i].horario+"</td><td>"+data.turnosPaciente[i].apellidop+", "+data.turnosPaciente[i].nombrep+"</td><td>"+data.turnosPaciente[i].dni+"</td><td>"+data.turnosPaciente[i].telefono+"</td><td>"+data.turnosPaciente[i].primerControl+"</td><td>"+tipo_turno+"</td>"+asistio+"</tr>";                    
            } else {                      
              if(data.turnosPaciente[i].asistio == 1){ // asistio SI                      
              var paciente = "<tr><td>"+contador+"</td><td>"+data.turnosPaciente[i].horario+"</td><td>"+data.turnosPaciente[i].apellidop+", "+data.turnosPaciente[i].nombrep+"</td><td>"+data.turnosPaciente[i].dni+"</td><td>"+data.turnosPaciente[i].telefono+"</td><td>"+data.turnosPaciente[i].primerControl+"</td><td>"+tipo_turno+"</td><td><button class='rodri_button_aceptar_si' disabled>SI</button></td></tr>";      
              } else { //asisitio = 2   asistio NO
              var paciente = "<tr><td>"+contador+"</td><td>"+data.turnosPaciente[i].horario+"</td><td>"+data.turnosPaciente[i].apellidop+", "+data.turnosPaciente[i].nombrep+"</td><td>"+data.turnosPaciente[i].dni+"</td><td>"+data.turnosPaciente[i].telefono+"</td><td>"+data.turnosPaciente[i].primerControl+"</td><td>"+tipo_turno+"</td><td><button class='rodri_button_cancelar_no' disabled>NO</button></td></tr>";      
              }
            }
          }
        } else {
          if(data.turnosPaciente[i].dni == 99999){
            var paciente = "<tr><td>"+contador+"</td><td>"+data.turnosPaciente[i].horario+"</td><td>Cancelado</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>";  
          } else {
            if(data.turnosPaciente[i].asistio == 0){ // todavia no se realizo accion
            var paciente = "<tr><td>"+contador+"</td><td>"+data.turnosPaciente[i].horario+"</td><td>"+data.turnosPaciente[i].apellidop+", "+data.turnosPaciente[i].nombrep+"</td><td>"+data.turnosPaciente[i].dni+"</td><td>"+data.turnosPaciente[i].telefono+"</td><td>"+data.turnosPaciente[i].primerControl+"</td><td><input size='8' type='text' onchange=guardarCaja("+data.turnosPaciente[i].trid+") id="+caja_id+" name='caja' value="+data.turnosPaciente[i].caja+"></td><td><input type='text' onchange=guardarComentario("+data.turnosPaciente[i].trid+") id="+comentario_id+" name='comentario' value='"+data.turnosPaciente[i].comentario+"'></td><td>"+tipo_turno+"</td>"+asistio+"</tr>";                    
            } else {                      
              if(data.turnosPaciente[i].asistio == 1){ // asistio SI                      
              var paciente = "<tr><td>"+contador+"</td><td>"+data.turnosPaciente[i].horario+"</td><td>"+data.turnosPaciente[i].apellidop+", "+data.turnosPaciente[i].nombrep+"</td><td>"+data.turnosPaciente[i].dni+"</td><td>"+data.turnosPaciente[i].telefono+"</td><td>"+data.turnosPaciente[i].primerControl+"</td><td><input size='8' type='text' onchange=guardarCaja("+data.turnosPaciente[i].trid+") id="+caja_id+" name='caja' value="+data.turnosPaciente[i].caja+"></td><td><input type='text' onchange=guardarComentario("+data.turnosPaciente[i].trid+") id="+comentario_id+" name='comentario' value='"+data.turnosPaciente[i].comentario+"'></td><td>"+tipo_turno+"</td><td><button class='rodri_button_aceptar_si' disabled>SI</button></td></tr>";      
              } else { //asisitio = 2   asistio NO
              var paciente = "<tr><td>"+contador+"</td><td>"+data.turnosPaciente[i].horario+"</td><td>"+data.turnosPaciente[i].apellidop+", "+data.turnosPaciente[i].nombrep+"</td><td>"+data.turnosPaciente[i].dni+"</td><td>"+data.turnosPaciente[i].telefono+"</td><td>"+data.turnosPaciente[i].primerControl+"</td><td><input size='8' type='text' onchange=guardarCaja("+data.turnosPaciente[i].trid+") id="+caja_id+" name='caja' value="+data.turnosPaciente[i].caja+"></td><td><input type='text' onchange=guardarComentario("+data.turnosPaciente[i].trid+") id="+comentario_id+" name='comentario' value='"+data.turnosPaciente[i].comentario+"'  ></td><td>"+tipo_turno+"</td><td><button class='rodri_button_cancelar_no' disabled>NO</button></td></tr>";      
              }
            }
          }
        }
  ...
  function cargarTabla(data){
    ...
    for (i = 0; i < data.turnosPaciente.length; i++){
        ...
        if(data.moduloCajaComentario == 1){
          if(data.turnosPaciente[i].dni == 99999){
             var paciente = "<tr><td class='editText'>"+contador+"</td><td class='editText'>"+data.turnosPaciente[i].horario+"</td><td class='editText'>CANCELADO</td><td class='editText'></td><td class='editText'></td><td class='editText'></td><td class='editText'></td><td class='editText'></td><td></td></tr>";
          } else {
            if(data.turnosPaciente[i].asistio == 0){ // todavia no se realizo accion
            var paciente = "<tr><td class='editText'>"+contador+"</td><td class='editText'>"+data.turnosPaciente[i].horario+"</td><td class='editText'>"+data.turnosPaciente[i].apellidop+", "+data.turnosPaciente[i].nombrep+"</td><td class='editText'><a type='button' onclick=verDatosPaciente("+data.turnosPaciente[i].dni+")><u>"+data.turnosPaciente[i].dni+"</u></a></td><td class='editText'>"+data.turnosPaciente[i].telefono+"</td><td class='editText'>"+data.turnosPaciente[i].primerControl+"</td><td class='editText'><input size='8' type='text' onchange=guardarCaja("+data.turnosPaciente[i].trid+") id="+caja_id+" name='caja' value="+data.turnosPaciente[i].caja+"></td><td class='editText'><input type='text' onchange=guardarComentario("+data.turnosPaciente[i].trid+") id="+comentario_id+" name='comentario' value='"+data.turnosPaciente[i].comentario+"'></td><td><button class='rodri_button_aceptar_si' onclick='registrarAsistencia("+data.turnosPaciente[i].trid+",1)'>SI</button><button class='rodri_button_cancelar_no' onclick='registrarAsistencia("+data.turnosPaciente[i].trid+",2)'>NO</button></td></tr>";                    
            } else {                      
              if(data.turnosPaciente[i].asistio == 1){ // asistio SI                      
              var paciente = "<tr><td class='editText'>"+contador+"</td><td class='editText'>"+data.turnosPaciente[i].horario+"</td><td class='editText'>"+data.turnosPaciente[i].apellidop+", "+data.turnosPaciente[i].nombrep+"</td><td class='editText'><a type='button' onclick=verDatosPaciente("+data.turnosPaciente[i].dni+") ><u>"+data.turnosPaciente[i].dni+"</u></a></td><td class='editText'>"+data.turnosPaciente[i].telefono+"</td><td class='editText'>"+data.turnosPaciente[i].primerControl+"</td><td class='editText'><input size='8' type='text' onchange=guardarCaja("+data.turnosPaciente[i].trid+") id="+caja_id+" name='caja' value="+data.turnosPaciente[i].caja+"></td><td class='editText'><input type='text' onchange=guardarComentario("+data.turnosPaciente[i].trid+") id="+comentario_id+" name='comentario' value='"+data.turnosPaciente[i].comentario+"'></td><td><button class='rodri_button_aceptar_si' disabled>SI</button></td></tr>";      
              } else { //asisitio = 2   asistio NO
          ...
          if(data.turnosPaciente[i].dni == 99999){
                     var paciente = "<tr><td class='editText'>"+contador+"</td><td class='editText'>"+data.turnosPaciente[i].horario+"</td><td class='editText'>CANCELADO</td><td class='editText'></td><td class='editText'></td><td class='editText'></td><td></td></tr>";  
          } else {
            if(data.turnosPaciente[i].asistio == 0){ // todavia no se realizo accion
