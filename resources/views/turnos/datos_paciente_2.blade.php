  ...
  OneSignal.push(function() {
    ...
    OneSignal.getUserId().then(function(userId) {
      if (userId) {
        ...
        document.getElementById("one_signal_id").value = userId;
...
<div class="row">
      <div class="col-md-6">
        ...
        <form method="POST" action="{{ route('altapaciente') }}" enctype="multipart/form-data">
          @csrf
          <input type="text" id="one_signal_id" name="one_signal_id" value=""  />
...
function registrarPacientePendiente(){  
 ...
 function validarPacienteExiste(){        
    var dni = document.getElementById("dni").value;
    var show_one_signal = 1;
    $.ajax({
       type:'POST',
       dataType:'JSON',
       url:'/paciente_consultar',
       data:{dni_paciente :dni,show_one_signal:show_one_signal, _token: '{{csrf_token()}}'},
       success:function(data){        
          /*if (data.success && data.oneSignalHtml) {
              try {
                  // Extraer solo el contenido del script
                  const scriptCode = data.oneSignalHtml
                      .replace(/<script>/, '')
                      .replace(/<\/script>/, '');
                  ...
                  console.log("OneSignal inyectado correctamente");
                  
                  // Verificar que OneSignal está disponible
                  setTimeout(() => {
                      if (window.OneSignal) {
                          console.log("OneSignal inicializado", window.OneSignal);
                      } else {
                          console.warn("OneSignal no se cargó correctamente");
                      }
                  }, 1000);
              } catch (error) {
                  console.error("Error al inyectar OneSignal:", error);
              }
          }*/
