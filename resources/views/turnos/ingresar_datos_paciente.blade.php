...
<div class="row">
      <div class="col-md-6">
        ...
        <form method="POST" action="{{ route('altapaciente') }}" enctype="multipart/form-data">
          ...
          <input type="hidden" id="mostrarCartelIpross" name="mostrarCartelIpross" value="0"  />
  ...
  <div class="col-md-6"> 
          <input type="hidden" id="moduloAfiliadoObligatorio" name="moduloAfiliadoObligatorio" value="{{$moduloAfiliadoObligatorio}}">                      
              ...
              <div class="form-check">                        
                ...
                <input onchange="validarDatos()" type="radio" id="check_afiliado_obligatorio_si" name="radio1" value="1" required>
                            
                <label class="form-check-label editText" for="materialChecked">No</label>
                <input onchange="validarDatos()" type="radio" id="check_afiliado_obligatorio_no" name="radio1" value="0" required>
              </div>
              <br>          

          <div id="seccionElegirObraSocial" class="form-group">
          <label for="obraSociales">* Obra Social</label>
          <select onchange="validarDatos()" class="form-control" id="obraSociales" name="obraSociales">            
            <option>N/A</option>            
            @foreach($obraSociales as $os)
            <option>{{$os->nombre}}</option>            
            @endforeach
          </select>
          <label id="label_obra_social_id" hidden class="letrasrojo">EL ESPECIALISTA NO TRABAJA CON ESTA OBRA SOCIAL</label>
          </div>
          ...
          <div hidden id="seccionTieneNoTieneFoto">
          ...
          <br>
          <label for="text" class="col-sm-0 control-label editText">* ¿Es primera consulta?</label>      <br>                  
          <div class="form-check">                        
            <label class="form-check-label editText" for="materialUnchecked">Si</label>
          ...
          <div class="form-check">
            ...
            <label class="form-check-label editText" for="exampleCheck1">He leído y acepto los </label><br>
...
@else  
  <div class="contenedor3">            
    <button hidden id="btnContinuar" hidden class="rodri_button contenido3 fontNav">Continuar</button>
  </div>
  </form>
  <div class="contenedor3">            
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body"> 
          <h4>Debe completar todos los campos.</h4><br>          
...
function validarObraSocial(obra_social) {
  ...
  $.ajax({
       ...
       success:function(data){                       
            if(data.response == 1) {                
              document.getElementById('label_obra_social_id').hidden = false;
            } else {                
              document.getElementById('label_obra_social_id').hidden = true;
            }
         }
    });
}
...
function validarDatos(){
      ...
      validarObraSocialIpross();
      validarObraSocial(obra_social);

      if(dni == null || (dni.localeCompare('')==0) || !soloNumeros(dni)) faltanDatos = 1;
      if(nombre == null || (nombre.localeCompare('')==0))faltanDatos = 1;
      if(apellido == null || (apellido.localeCompare('')==0))faltanDatos = 1;
      if(telefono == null || (telefono.localeCompare('')==0))faltanDatos = 1;
      if(fecha_nacimiento_dia == null || (fecha_nacimiento_dia.localeCompare('')==0))faltanDatos = 1;
      if(fecha_nacimiento_mes == null || (fecha_nacimiento_mes.localeCompare('')==0))faltanDatos = 1;
      if(fecha_nacimiento_anio == null || (fecha_nacimiento_anio.localeCompare('')==0) || (fecha_nacimiento_anio.length != 4)){
        faltanDatos = 1;
      }
      if(domicilio == null || (domicilio.localeCompare('')==0))faltanDatos = 1;
      if(localidad == null || (localidad.localeCompare('')==0))faltanDatos = 1;
      if(mail == null || (mail.localeCompare('')==0))faltanDatos = 1;
      if(document.getElementById("check_afiliado_obligatorio_si").checked == 0 && document.getElementById("check_afiliado_obligatorio_no").checked == 0)faltanDatos = 1;
      if(document.getElementById("check_si").checked == 0 && document.getElementById("check_no").checked == 0)faltanDatos = 1;
      if(obra_social == null || (obra_social.localeCompare('N/A')==0))faltanDatos = 1;
      if(obra_social.localeCompare('PARTICULAR')!=0){
        if(numero_afiliado == null || (numero_afiliado.localeCompare('')==0))faltanDatos = 1;
      }

      if(terminos_condiciones == false )faltanDatos = 1;
...
function registrarPacientePendiente(){  
 ...
 function validarPacienteExiste(){        
  ...
  function validarObraSocialIpross(){
      var medico_id = document.getElementById("medico_id").value;
      var obraSocial = document.getElementById("obraSociales").value;      
      var mostrarCartelIpross = document.getElementById("mostrarCartelIpross").value;
      if(mostrarCartelIpross == 0 && medico_id == 15 && obraSocial.localeCompare("IPROSS") == 0 ) {
        document.getElementById("mostrarCartelIpross").value = 1;
        var telefono_consultorio = document.getElementById("consultorio_telefono").value;
        var p = document.getElementById("modal_texto_medico_obra_social");                        
        p.innerHTML = "El especialista tiene convenio con la obra social IPROSS. Para ello es necesario presentar una Orden especial, por favor para obtener más información comunicarse con el consultorio TEL: "+telefono_consultorio+". <br> Muchas Gracias";
        $('#modalNoTrabajaObraSocial').modal();
      }
  }
  ...
  function vaciarCampos() {
    ...
    document.getElementById("seccionTieneFoto").hidden = true;
    document.getElementById("seccionTieneNoTieneFoto").hidden = false;      
    document.getElementById("obraSociales").value = "N/A";          
  }
